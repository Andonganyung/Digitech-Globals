# Fix blog and services page logos - correct path and add tagline

$folders = @("blog", "services")

foreach ($folder in $folders) {
    $files = Get-ChildItem -Path "C:\Users\Andong\Digitech-Globals\$folder" -Filter "*.html"
    
    # Pattern with wrong path
    $pattern1 = '<img src="images/dg-logo.png" alt="DG Logo" class="logo-icon">'
    $replacement1 = '<img src="../images/dg-logo.png" alt="DG Logo" class="logo-icon">'
    
    # Pattern without tagline structure
    $pattern2 = @'
<img src="../images/dg-logo.png" alt="DG Logo" class="logo-icon">
                <span>DigiTech<strong>Globals</strong></span>
'@
    
    $replacement2 = @'
<img src="../images/dg-logo.png" alt="DG Logo" class="logo-icon">
                <div class="logo-text">
                    <span>DigiTech<strong>Globals</strong></span>
                    <span class="logo-tagline">Engineering the Future of IT</span>
                </div>
'@
    
    foreach ($file in $files) {
        $content = Get-Content $file.FullName -Raw -Encoding UTF8
        $updated = $false
        
        # Fix path first
        if ($content -match [regex]::Escape($pattern1)) {
            $content = $content -replace [regex]::Escape($pattern1), $replacement1
            $updated = $true
        }
        
        # Then add tagline
        if ($content -match [regex]::Escape($pattern2)) {
            $content = $content -replace [regex]::Escape($pattern2), $replacement2
            $updated = $true
        }
        
        if ($updated) {
            Set-Content -Path $file.FullName -Value $content -NoNewline -Encoding UTF8
            Write-Host "Updated: $($file.FullName)"
        }
    }
}

Write-Host "`nDone!"
